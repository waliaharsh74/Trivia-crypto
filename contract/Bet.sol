...
contract CryptoTriviaBet {
    ...
    event BetCreated(string slug, address indexed creator, uint256 amount, string topic);
    ...
    function createBet(string calldata slug, string calldata topic) external payable {
    ...
    function submitScore(string calldata slug, uint8 score) external betExists(slug) {
        Bet storage b = bets[_slugHash(slug)];
        require(msg.sender == b.creator || msg.sender == b.joiner, "Bet: not a participant");
        require(score <= 15, "Bet: max score 15");  

        if (msg.sender == b.creator) {
            require(!b.creatorCompleted, "Bet: creator already submitted");
            b.creatorScore = score;
            b.creatorCompleted = true;
        } else {
            require(!b.joinerCompleted, "Bet: joiner already submitted");
            b.joinerScore = score;
    ...
    function resolve(string calldata slug) external betExists(slug) {
        Bet storage b = bets[_slugHash(slug)];
        require(b.creatorCompleted && b.joinerCompleted, "Bet: quizzes not done");
    ...
    function getBet(string calldata slug) external view betExists(slug) returns (Bet memory) {
